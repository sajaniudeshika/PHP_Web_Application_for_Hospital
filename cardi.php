...
$query="select * from doctor where specialized_area='Cardiologist'";
$query2 = "SELECT image from login where abstype='Cardiologist'";
    ...
    <div class="row">
          <div class="col-md-3">
            <div class="card" style="margin-top:10px; margin-left:10px;">
              <?php
                   $con=mysqli_connect("localhost","root","","suwapiyasa");
                   $query = "SELECT image from login where abstype='Cardiologist'";
                   $result = mysqli_query($con,$query);
                   while($row= mysqli_fetch_array($result)){
              ...
              ?>
               <div class="container">
                 ...
                 <?php
            					...
            					$date =date("Y-m-d");
            					$sql="SELECT count(nic_no) AS total FROM appointment WHERE doctor='Cardiologist' and date='$date'";
          ...
          <div class="col-md-8">
            <div class="sub_nav">
              <ul>
                ...
                <li id="view_his"><a href="cardi-patienthistory.php" style="text-decoration:none;color:#fff">Patient History</a></li>
              </ul>
            </div>
              ...
              <div id="appoitments" style="display:none;">
                  						...
                  						<thead>
                  						  ...
                  						  <tbody>
                  							<?php
                  								$con=mysqli_connect("localhost","root","","suwapiyasa");
																	$date =date("Y-m-d");
																	$query="select * from appointment WHERE doctor='Cardiologist' AND date>='$date'";
                  								$result=mysqli_query($con,$query);
                  								while($row=mysqli_fetch_array($result)){
                  								$fname = $row['fname'];
                  								$lname = $row['lname'];
                ...
                <div id="mg-profile" style="display:none;">
                  ...
                  <form class="form-group" action="cardifunc.php" method="post" enctype="multipart/form-data">
                ...
                <div id="patients" style="display:none;">
                  <form class="form-group" action="" method="post">
                          ...
                          if(isset($_POST['register'])){
                            ...
                            $date = date("Y-m-d");
                            $query = "INSERT INTO apmt_cardi(fname,lname,nic_no,contact,priscription,date)values('$fname','$lname','$nic','$phone','$priscription','$date')";
                            $result=mysqli_query($con,$query);
                            if($result){
                              echo "<script>alert('Registered')</script>";
                              echo "<script>window.open('cardi.php','_self')</script>";
                            }else{
